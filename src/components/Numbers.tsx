...
const StatCard: React.FC<StatCardProps> = ({ value, label, icon: Icon, startAnimation, color }) => {
    ...
    return (
        <div className="group relative bg-white  p-3 rounded-2xl border border-gray-100 shadow-lg hover:shadow-2xl transition-all duration-300 ease-out hover:-translate-y-2">
            ...
            <div className="relative flex flex-col items-center justify-center text-center">
                ...
                <div className={`mb-4 p-4 rounded-full bg-opacity-10 ${color.replace('bg-', 'bg-opacity-10 text-')}`}>
                    <Icon className={`w-12 h-12 ${color.replace('bg-', 'text-')}`} />
                </div>
                ...
                {/* Label */}
                <p className="text-[#5C4033] font-medium text-lg uppercase tracking-wide">
                    {label}
                </p>
            </div>
        </div>
